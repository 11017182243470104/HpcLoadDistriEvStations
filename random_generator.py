...
def create_rand_instance():
    ...
    for c in all_cpos:
        ...
        for i in range(y):
			...
			all_cpos[c].set_attr("CapConMax", all_cpos[c].get_attr("CapConMax")
                                 + all_cpos[c].get_customers()[c + str(i)].get_attr("CSPowMax"))
			all_cpos[c].set_attr("CapConMin", all_cpos[c].get_attr("CapConMin")
                                 + all_cpos[c].get_customers()[c + str(i)].get_attr("CSPowMin"))
        all_cpos[c].set_attr("CapConMax",
                             all_cpos[c].get_attr("CapConMax") + 120 * (x - len(all_cpos[c].get_customers())))
    ...
    table_data_cust = [["Customer", "CPO", "BEVPowMax", "CSPowMin", "CSPowMax", "BEVcap_rest", "Exp. time", "Eta"]]

    for c in all_cpos:
        for i in all_cpos[c].get_customers():
            table_data_cust.append([all_cpos[c].get_customers()[i].get_attr("id"),
                                    all_cpos[c].get_customers()[i].get_attr("cust_cpo"),
                                    format(all_cpos[c].get_customers()[i].get_attr("BEVPowMax"), ".2f"),
                                    format(all_cpos[c].get_customers()[i].get_attr("CSPowMin"), ".2f"),
                                    format(all_cpos[c].get_customers()[i].get_attr("CSPowMax"), ".2f"),
                                    format(all_cpos[c].get_customers()[i].get_attr("BEVcap_rest"), ".2f"),
                                    format(all_cpos[c].get_customers()[i].get_attr("time_exp"), ".2f"),
                                    format(all_cpos[c].get_customers()[i].get_attr("eta"), ".2f")
                                    ])
